...
async function fetchAndRender() {
  ...
  const [aRes, hwRes, meRes] = await Promise.all([fetch(`${API_BASE}/attendance`, {
    headers
  }), fetch(`${API_BASE}/hwafez`, {
    headers
  }), fetch(`${API_BASE}/me`, {
    headers
  })]);
...
async function showHwafez() {
  try {
    ...
    document.getElementById('hwafezSection').scrollIntoView({
      behavior: 'smooth'
    });
  } catch (e) {
...
function logout() {
  ...
  ['records', 'pushSection', 'hwafezSection'].forEach(id => document.getElementById(id).classList.add('hidden'));
