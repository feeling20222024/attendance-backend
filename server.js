...
if (!JWT_SECRET || !SUPERVISOR_CODE || !SHEET_ID || !GOOGLE_SERVICE_KEY || !FIREBASE_SERVICE_ACCOUNT) {
  console.error('❌ بعض متغيرات البيئة مفقودة (JWT_SECRET, SUPERVISOR_CODE, GOOGLE_SHEET_ID, GOOGLE_SERVICE_KEY, FIREBASE_SERVICE_ACCOUNT)');
  process.exit(1);
}
...
const app = express();
app.use(cors({ allowedHeaders:['Content-Type','Authorization'], exposedHeaders:['Authorization'], origin: true }));
app.use(express.json());
app.use(express.static(path.join(__dirname,'public')));

// 6) تهيئة Google Sheets من env
let sheetCreds;
try {
  sheetCreds = JSON.parse(GOOGLE_SERVICE_KEY);
} catch (e) {
  console.error('❌ GOOGLE_SERVICE_KEY غير صالح:', e);
  process.exit(1);
}
...
app.get('/api/attendance',authenticate,async(req,res)=>{ try{
...
app.get('/api/hwafez',authenticate,async(req,res)=>{ try{
...
app.get('/api/tqeem',authenticate,async(req,res)=>{ try{
...
app.post('/api/register-token',authenticate,(req,res)=>{ const{user,token}=req.body; if(!user||!token) return res.status(400).json({error:'user and token required'}); tokens.set(token,user); res.json({success:true}); });

app.post('/api/notify-all',authenticate,async(req,res)=>{ if(req.user.code!==SUPERVISOR_CODE) return res.status(403).json({error:'Forbidden'}); const{title,body}=req.body; await Promise.allSettled(Array.from(tokens).map(([t])=>sendPushTo(t,title,body))); res.json({success:true}); });

const userNotifications={};
app.post('/api/notifications',authenticate,(req,res)=>{ const{title,body,time}=req.body; if(!title||!body||!time) return res.status(400).json({error:'Missing fields'}); const code=req.user.code; userNotifications[code]=userNotifications[code]||[]; userNotifications[code].unshift({title,body,time}); if(userNotifications[code].length>50) userNotifications[code].pop(); res.json({success:true}); });
app.get('/api/notifications',authenticate,(req,res)=>res.json({notifications:userNotifications[req.user.code]||[] }));
