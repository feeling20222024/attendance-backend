...
// 1) نحمّل متغيّرات البيئة (بيقرأ ملف .env أو أي متغيّرات عرَّفتها على Render)
...
// —————————————————————————————————————————————————————————————
// 3) نقرّأ متغيّرات البيئة الأساسية
// —————————————————————————————————————————————————————————————
const {
  JWT_SECRET,
  SUPERVISOR_CODE,
...
if (!GOOGLE_SERVICE_KEY) {
  console.error('❌ خطأ: متغيّر GOOGLE_SERVICE_KEY غير موجود.');
  process.exit(1);
}
if (!FIREBASE_SERVICE_ACCOUNT) {
  console.error('❌ خطأ: متغيّر FIREBASE_SERVICE_ACCOUNT غير موجود.');
  process.exit(1);
}
...
try {
  ...
  console.error('❌ خطأ: متغيّر GOOGLE_SERVICE_KEY ليس بصيغة JSON صالحة.');
  process.exit(1);
}
...
try {
  ...
  console.error('❌ خطأ: متغيّر FIREBASE_SERVICE_ACCOUNT ليس بصيغة JSON صالحة.');
  process.exit(1);
}
...
// —————————————————————————————————————————————————————————————
// 5) دوال الوصول إلى Google Sheets (نستخدم v3.3.0 لعدم وجود useServiceAccountAuth في الإصدارات الحديثة)
// —————————————————————————————————————————————————————————————
...
async function readSheet(title) {
  ...
  // نحول كل صف إلى مصفوفة قيم
  const data    = rows.map(r => headers.map(h => r[h] ?? ''));
  return { headers, data };
}

// —————————————————————————————————————————————————————————————
// 6) دالة الوسيط (middleware) للتحقّق من JWT
// —————————————————————————————————————————————————————————————
...
app.post('/api/login', async (req, res) => {
  ...
  // نطبّق تطبيع الأرقام (normalizeDigits) على ما أرسله المستخدم
  ...
  try {
    ...
    // نبحث الصفِّ المطابق في البيانات بعد تطبيع الأرقام أيضًا
...
// —————————————————————————————————————————————————————————————
// 9) مسار إرجاع بيانات “المستخدم الحالي” (/api/me)
// —————————————————————————————————————————————————————————————
app.get('/api/me', authenticate, async (req, res) => {
  try {
    const { headers, data } = await readSheet('Users');
    const idxCode = headers.indexOf('كود الموظف');
    // نطبّق normalizeDigits على كود المستخدم
...
// —————————————————————————————————————————————————————————————
// 10) مسار /api/attendance (محميّ بJWT)
// —————————————————————————————————————————————————————————————
...
// —————————————————————————————————————————————————————————————
// 12) مسار تسجيل توكن FCM (مؤقتًا في Map)
// —————————————————————————————————————————————————————————————
